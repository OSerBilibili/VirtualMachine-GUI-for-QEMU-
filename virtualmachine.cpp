...
void VirtualMachine::createMachine() {
    ...
    command += (machine.format + QString::fromUtf8(" ") + machine.path + machine.name + QString::fromUtf8(".img ") + machine.size);
    QProcess p(0);
    p.start(command);
    p.waitForFinished();
}
...
void VirtualMachine::runMachine() {
    ...
    QString command = ui->lineEditRun->text() + space + QString::fromUtf8("-enable-kvm -usb -soundhw all -m ")\
